...
def calculate_wss(frames):
    ...
    for i in range(len(gray)-1):
        ...
        wss.append(np.where(small, mu * np.sqrt(du**2 + dv**2) / pixel_size_m, 0))
    return wss
...
if video:
    ...
    if st.button("解析を実行"):
        with st.spinner("解析中..."):
            ...
            b1, b2 = st.columns(2)
            with b1:
                st.pyplot(fig_be_w); st.markdown(get_high_sectors(arr_w, "WSS"))
            with b2:
                st.pyplot(fig_be_p); st.markdown(get_high_sectors(arr_p, "Pressure"))

            # Summary
            st.markdown("### 🧠 Summary")
            with st.expander("WSSとPressureの説明"):
                st.markdown("- **WSS**：血管壁にかかるずり応力。高値は血管障害を示す。")
                st.markdown("- **Pressure**：模擬内圧。高値は血流抵抗の増加を示唆。")
            ...
            # Combined judgment with accent style
            sev_color = "#dff5e1" if comment == "異常なし" else "#fff6db" if "可能性" in comment else "#ffe5e5"
            st.markdown(f"""
            <div style="
              background:{sev_color};
              border-left:6px solid {'#34c759' if comment=='異常なし' else '#e6a23c' if '可能性' in comment else '#ed3f14'};
              padding:12px 20px;
              border-radius:8px;
              font-weight:600;
              font-size:16px;
