    ...
    if st.button("解析を実行"):
        with st.spinner("解析中..."):
            ...
            with st.expander("🛈 コメント説明"):
                desc_map = {
                    "異常なし": "WSS・Pressure共に正常範囲内です。",
                    "軽度の上昇傾向": "WSSまたはPressureのどちらかがやや高めです。"
                                      "明らかなリスクは少なめですが、モニタリング推奨。",
                    "中等度の上昇傾向": f"WSS時間比率: {wsr}%、Pressure時間比率: {pr}% で中等度に上昇。"
                                      "特に{ 'WSS' if wsr>pr else 'Pressure' }側の負荷が顕著です。",
                    "WSS極端に高い": f"WSSが非常に高く、最大WSSは {wss_max} Pa。"
                                    "血管壁に大きなずり応力がかかっています。",
                    "Pressure極端に高い": f"Pressureが非常に高く、最大Pressureは {p_max}。"
                                         "内圧負荷が強く、抵抗の増加が懸念されます。",
                    "重度の狭窄の疑い": f"WSS({wsr}%)・Pressure({pr}%)共に高値。"
                                     "狭窄の可能性が高く、追加評価が推奨されます。",
                    "データ不足の可能性あり": "赤色領域の検出が不足しているため、"
                                        "対象部位がうまく捉えられていない可能性があります。"
                }
                st.write(desc_map.get(comment, "該当コメントの詳細なし。"))
